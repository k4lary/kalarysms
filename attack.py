...
pause() {
    read -n1 -r -p "Tkaya enter bka ..." key
}
banner() {
    clear
    echo -e "\e[1;31m"
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing KaLaRy'
    else
        figlet KaLaRy
    fi
    ...
    echo -e "\e[1;32m           Telegram: KaLaRy F"
    echo -e "\e[4;32m   Instagram: kal4ry"
    echo -e" facebook: Ahmed Mahmood Mahanes"
...
do
    ...
    clear
    if [ $ch -eq 1 ];then
        $PYTHON kalary.py --sms
        exit
    elif [ $ch -eq 2 ];then
        $PYTHON kalary.py --call
        exit
    elif [ $ch -eq 3 ];then
        $PYTHON kalary.py --mail
        exit
    elif [ $ch -eq 4 ];then
        echo -e "\e[1;34m Downloading Latest Files..."
        rm -f .update
        $PYTHON kalary.py --update
        echo -e "\e[1;34m RUN kalary Again..."
        pause
        exit
    elif [ $ch -eq 5 ];then
        banner
